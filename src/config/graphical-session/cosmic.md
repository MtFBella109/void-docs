...
Install the `cosmic-desktop-minimal` package, if you just want all cosmic components and the dependencies, that it runs at all. Optionally you can install `cosmic-desktop-full`, wich comes with bluetooth, fonts and set up's xdg-user-dirs, pipewire to work correctly with COSMIC. 

To use the "Networks" widget, install `NetworkManager` and enable the 
